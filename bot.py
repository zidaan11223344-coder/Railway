...
SUPABASE_URL = str(C.get("supabase_url") or os.environ.get("SUPABASE_URL", "")).strip()
SUPABASE_KEY = str(C.get("supabase_key") or os.environ.get("SUPABASE_KEY", "")).strip()
USERNAME = str(C.get("username") or os.environ.get("BOT_USERNAME", "")).strip()
PASSWORD = str(C.get("password") or os.environ.get("BOT_PASSWORD", "")).strip()

if not SUPABASE_URL or not SUPABASE_KEY or not USERNAME or not PASSWORD:
    log.error("نقص في إعدادات الاتصال (Supabase URL, Key, Username, Password مفقودة في config.json أو Environment Variables)")
    sys.exit(1)

OWNER = (C.get("owner_username") or os.environ.get("OWNER_USERNAME") or USERNAME).strip().lower()
POLL = max(1.0, float(C.get("poll_seconds", os.environ.get("POLL_SECONDS", 2))))
SEARCH_URL = C.get("music_search_url") or os.environ.get("MUSIC_SEARCH_URL", "https://giant-chat-app.lovable.app/api/public/search-track")
...
MEDIA_GATEWAY_URL = str(C.get("media_gateway_url") or os.environ.get("MEDIA_GATEWAY_URL", "http://127.0.0.1:8080")).strip().rstrip("/")
